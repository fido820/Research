...
\begin{columns}[t,totalwidth=\twocolwid]
	...
	\begin{column}{\onecolwid}
		\begin{block}{Reinforcement Learning}
			\begin{itemize}
				...
				\item Ordinarily, the Q-function is modeled by storing state-action pairs in a table, but this is impractical for large state spaces
				\begin{itemize}
					\item Use a function approximator instead to model the Q-function: \textbf{Artificial Neural Networks}
				\end{itemize}
				\item With regular Q-Learning, no relation is made between similar states or actions
				\begin{itemize}
		...
		\begin{block}{Action Selection}
			\begin{itemize}
				...
				\begin{itemize}
					\item Use a Softmax Probability Distribution to select an action
				\end{itemize}
				\item Cannot just hardcode the exploration level, Fido should stop exploring if confident in its actions and explore more if it is being retrained
				\begin{itemize}
					\item Measure confidence using changes in the neural network weight matrix and adjust exploration accordingly
				\end{itemize}
			\end{itemize}
		\end{block}
	\end{column}
	...
	\begin{itemize}
		\item Fido needs to update its model after receiving reward
		\begin{itemize}
			\item Use Stochastic Gradient Descent to generate a new action to expected reward function for the past state
			\item Train the neural network to output the data points that define this function
		\end{itemize}
		\item Many neural network training algorithms require a learning rate that depends on the specifics of the task
		\begin{itemize}
			\item Use the new Adadelta training method which dynamically adjusts learning rate, decreases latency, and results in better neural network accuracy
		\end{itemize}
		...
		\begin{itemize}
			\item Allows training of action sequences for complex tasks
		\end{itemize}
		\item Fido's optimal neural network architecture varies between tasks. Ex. larger networks are more suited for complicated tasks.
		\begin{itemize}
			\item Efficiently grow and prune Fido's neural network using neuron sensitivity approximations gathered from fluctuations in the network's weights.
		\end{itemize}
	\end{itemize}
